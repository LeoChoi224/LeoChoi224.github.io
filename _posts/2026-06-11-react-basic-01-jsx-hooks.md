...
## Component 분리
React에서는 특정 상태가 변경될 때 컴포넌트 **전체가 다시 렌더링되는** 문제가 발생할 수 있다.

불필요한 전체 렌더링을 방지하고 애플리케이션의 성능을 방어하기 위해서는 기능과 역할을 기준으로 컴포넌트를 **쪼개어 독립적으로 그려야 한다.**
...
컴포넌트가 다시 렌더링될 때마다 불필요한 계산이 반복되는 것을 줄이기 위해 사용한다.
